...
export function useInstorePurchase(
  onApproved?: (payment: PixPaymentStatusResponse) => void,
) {
  ...
  const pollStatus = useCallback(
    async (paymentId: number) => {
      try {
        const nextStatus = await paymentService.getInstorePaymentStatus(paymentId);
        ...
        setError(getApiErrorMessage(err, "Erro ao verificar pagamento presencial"));
      }
    },
  ...
  const startPurchase = useCallback(
    async (coinPackageId: number) => {
      try {
        ...
        const result = await paymentService.createInstorePurchase(coinPackageId);
        setPurchase(result);
        setStatus({
          id: result.paymentId,
        ...
        setError(getApiErrorMessage(err, "Erro ao criar pagamento presencial"));
      } finally {
        setLoading(false);
      }
    },
